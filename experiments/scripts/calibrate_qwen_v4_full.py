...
OUTPUT_FILE="calibration_qwen_v4_full.json"
...
def run():
    ...
    results=[]
    print(f"\n>>> SWEEP COMPLET — {MODEL} — 3×7×10 = 210 appels\n")
    for fk in ["A","B","C"]:
        print(f"\n{'='*50}\nFamily {fk}\n{'='*50}")
        for lv in LEVELS:
    ...
    for fk in ["A","B","C"]:
        ...
        if sl<-0.0001 and n_down>=4:
            print(f"    a_secteur  = {ic:.4f}")
            print(f"    beta_secteur = {-sl:.6f}")

    with open(OUTPUT_FILE,"w") as f:
        json.dump({"meta":{"protocol":"v4.3_qwen_full","model":MODEL,
                           "role":"intermediate instruct model",
