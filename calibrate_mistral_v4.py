...
MAX_CONTEXT=32_768;TEMPERATURE=0.0;MAX_TOKENS=12;RUNS_PER_LEVEL=5
OUTPUT_FILE="calibration_v4_results.json"
...
def family_a(n):
    # Clé du fix : le code est mis sur LA MÊME LIGNE que chaque mention,
    # mais Vega apparaît 2x : une fois sans code, une fois avec.
    # La consigne dit explicitement : cherche le dept sans code, trouve son code ailleurs.
    decoys=[("Orion","KQ-17"),("Nyx","TR-91"),("Cetus","WP-04"),("Draco","SN-66")]
    random.shuffle(decoys)
    L=["[SECTION REGISTRY]"]
    # Première mention de Vega : sans code
    L.append("Dept: Vega | region: west | status: active")
    for d,c in decoys[:2]:
        L.append(f"Dept: {d} | code: {c} | status: active")
    L+=["",noise(n//2),"","[SECTION CODES]"]
    # Deuxième mention de Vega : avec code
    L.append("Dept: Vega | code: LM-22 | status: active")
    for d,c in decoys[2:]:
        L.append(f"Dept: {d} | code: {c} | status: active")
    L+=["",noise(n-n//2)]
    q=f"""{CONTRACT}
...
def family_b(n):
    ...
    for s,col,p in entries:
        ...
        if random.random()<0.4: L.append(noise(max(1,n//len(entries))))
    L.append(noise(n//2))
    q=f"""{CONTRACT}
...
def family_c(n):
    ...
    for role,vf,vu,auth in entries:
        ...
        if random.random()<0.3: L.append(noise(2))
    L+=["",noise(n)]
    q=f"""{CONTRACT}
...
LEVELS_FULL=[{"level":1,"n":0},{"level":2,"n":3},{"level":3,"n":8},{"level":4,"n":18},
             {"level":5,"n":40},{"level":6,"n":80},{"level":7,"n":140}]
LEVELS_MINI=[{"level":1,"n":0}]

def run_levels(client, families, levels, results):
    for fk in families:
        print(f"\n{'='*50}\nFamily {fk}\n{'='*50}")
        for lv in levels:
            ...
            for i in range(RUNS_PER_LEVEL):
                ...
                print(f"    run {i+1}: {tag} | {sc['raw']!r} | c_dyn={cd:.1f}")
                time.sleep(0.3)
            ...
            results.append({"family":fk,"rule":item["rule"],"level":level,"noise":n,
                            "avg_prompt_tokens":round(avg_pt),"avg_c_dyn":round(avg_cd,4),
            ...
            print(f"    → sr={sr:.0%} | c_dyn={avg_cd:.2f} | log_tau={lt:.3f}")

def print_fits(results):
    print(f"\n{'='*50}\nFits\n{'='*50}")
    fits={}
    for fk in ["A","B","C"]:
        rows=[r for r in results if r["family"]==fk]
        if len(rows)<2: continue
        xs=np.array([r["avg_c_dyn"] for r in rows]);ys=np.array([r["log_tau_obs"] for r in rows])
        sl,ic=np.polyfit(xs,ys,1)
        fits[fk]={"slope":round(float(sl),6),"intercept":round(float(ic),4)}
        print(f"  Family {fk}: slope={sl:+.6f} {'✓ dégradation' if sl<-0.0001 else '⚠ plat'}")
    return fits

def run():
    client=OpenAI(base_url=os.environ.get("OPENAI_BASE_URL","http://74.48.78.46:24810/v1"),api_key="none")
    results=[]
    print("\n>>> PHASE 1 : mini rerun niveau 1 (diagnostic)")
    run_levels(client, ["A","B","C"], LEVELS_MINI, results)
    print("\n>>> Résultats niveau 1 :")
    for r in results:
        print(f"  Family {r['family']} L1 : sr={r['success_rate']:.0%}")
    go=input("\n>>> Continuer vers le sweep complet ? (o/n) : ").strip().lower()
    if go!="o":
        with open("calibration_v4_mini.json","w") as f: json.dump({"results":results},f,indent=2)
        print("→ calibration_v4_mini.json sauvegardé. Arrêt."); return
    print("\n>>> PHASE 2 : sweep complet")
    results=[]
    run_levels(client, ["A","B","C"], LEVELS_FULL, results)
    fits=print_fits(results)
    with open(OUTPUT_FILE,"w") as f:
        json.dump({"meta":{"protocol":"v4.3","model":MODEL,
                           "total_calls":3*len(LEVELS_FULL)*RUNS_PER_LEVEL},
