...
class EgoProfiler:
    def __init__(self, model: str | ModelSectorConfig):
        if isinstance(model, str):
            if model not in SECTOR_CONFIGS:
                raise ValueError(f"Unknown model '{model}'. Available: {list(SECTOR_CONFIGS.keys())}")
            self.config = SECTOR_CONFIGS[model]
        else:
            self.config = model

    def _eta(self, tokens):
        return min(max(tokens / self.config.max_context_tokens, 0.0), 1.0)

    def get_spectatorization_ratio(self, tokens):
        return round(self._eta(tokens) ** 1.5, 6)

    def get_geometric_saturation(self, tokens):
        eta   = self._eta(tokens)
        r_eta = R_1D + (R_HOLO - R_1D) * eta
        status = "Critical" if r_eta >= 1.55 else "Warning" if r_eta > 1.45 else "Safe"
        return {"r_eta": round(r_eta, 6), "eta": round(eta, 6), "status": status, "r_1d": R_1D, "r_holo": R_HOLO}

    def estimate_logical_decay(self, tokens):
    ...
    def profile(self, prompt_tokens: int) -> ProfileResult:
        ...
        return ProfileResult(
            model=self.config.name, prompt_tokens=prompt_tokens,
            max_context_tokens=self.config.max_context_tokens,
            alpha_s=alpha_s, eta=geom["eta"], r_eta=geom["r_eta"],
            geometric_regime=geom["status"], tau=tau, c_dyn=c_dyn,
        )
